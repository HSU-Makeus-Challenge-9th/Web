...
//       <img src={data?.data?.avatar as string} alt="프로필 아바타" />
...
import { getMyInfo, updateMyInfo } from "../apis/auth"; // 🚨 updateMyInfo API 사용
import { useAuth } from "../context/AuthContext";
import type { ResponseMyInfoDto } from "../types/auth";
// 🚨 UpdateMyInfoDto 타입 정의가 필요합니다. 이 파일에 임시 정의합니다.
// 실제로는 /src/types/auth.ts에 정의되어야 합니다.
...
const ProfileEditForm: React.FC<ProfileEditFormProps> = ({
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      onSuccess(); // 마이페이지 데이터 refetch
      onClose(); // 폼 닫기
    },
    onError: (err: any) => {
      console.error("프로필 수정 실패:", err);
      // 🚨 서버 응답 에러 메시지를 사용하도록 처리
      const errorMessage =
