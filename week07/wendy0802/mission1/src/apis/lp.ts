...
export const lpAPI = {
  toggleLike: async (lpId: number): Promise<{ status: boolean }> => {
    const accessToken = localStorage.getItem("accessToken");
    if (!accessToken) {
      throw new Error("로그인이 필요합니다.");
    }
    // 서버 스펙에 맞춰 메서드/경로를 조정하세요. 일반적으로 POST/DELETE 또는 POST 토글.
    const response = await fetch(`${API_BASE_URL}/v1/lps/${lpId}/likes`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${accessToken}`,
    ...
    if (!response.ok) {
      ...
      throw new Error(errBody?.message ?? "좋아요 처리에 실패했습니다.");
    }
