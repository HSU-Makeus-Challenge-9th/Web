...
import type { LpDetailsItem } from "../../types/lp";
import usePatchLpInfo from "../../hooks/queries/usePatchLpInfo";

interface LpEditModalProps {
  onClose: () => void;
  lpData: LpDetailsItem;
}

const LpEditModal = ({ onClose, lpData }: LpEditModalProps) => {
  const [lpName, setLpName] = useState(lpData.title);
  const [lpContent, setLpContent] = useState(lpData.content);
  const [lpTag, setLpTag] = useState("");
  const [tags, setTags] = useState<string[]>(
    lpData.tags.map((tag) => tag.name)
  );
  const [thumbnail, setThumbnail] = useState<string | null>(lpData.thumbnail);
  const fileInputRef = useRef<HTMLInputElement>(null);
  const { mutate, isPending } = usePatchLpInfo();
    ...
    mutate(
      {
        ...
        lpItem: {
          title: lpName,
          content: lpContent,
          thumbnail: thumbnail || lpData.thumbnail,
          tags: tags,
          published: true,
        },
      },
  ...
  return (
    <div className="flex items-center justify-center" onClick={onClose}>
      ...
      >
        ...
        >
          <img
            src={thumbnail || lpData.thumbnail}
