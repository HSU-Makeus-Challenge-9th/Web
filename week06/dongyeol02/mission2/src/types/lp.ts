...
// 댓글 목록 API 응답의 Data 필드 (useInfiniteQuery의 Page 타입)
export interface LpCommentApiData {
  data: Comment[]; // 실제 댓글 배열
  nextCursor: number | null;
  hasNext: boolean;
}

// 댓글 목록 API 전체 응답 구조
export interface LpCommentFullResponse {
  status: boolean;
  message: string;
  statusCode: number;
  data: LpCommentApiData; // 이 안에 data: [...] 와 nextCursor, hasNext가 있음
}
