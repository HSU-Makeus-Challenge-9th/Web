...
export const LPCreationModal: React.FC<LPCreationModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // 1️⃣ 이미지 업로드 (서버에 파일 보내기 → URL 받기)
      const imageUrl = await uploadImage(thumbnailFile);

      console.log("🚀 업로드 성공! imageUrl:", imageUrl);

      // 2️⃣ LP 생성 Payload 구성
      const payload: CreateLpDto = {
        title: title.trim(),
        content: content.trim(),
        thumbnail: imageUrl, // 서버에서 받은 URL
        tags: tags,
        published: true,
      };
      ...
      console.error("LP 생성 실패: 이미지 업로드 단계에서 오류 발생:", error);
