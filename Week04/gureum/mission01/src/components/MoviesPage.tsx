...
const MoviesPage: React.FC<MoviesPageProps> = ({ endpoint, title }) => {
  ...
  const baseUrl = `https://api.themoviedb.org/3/movie/${endpoint}?language=ko-KR&page=${currentPage}`;

  const { data, loading, error } = useCustomFetch<MovieResponse>(
    API_TOKEN ? baseUrl : null,
    {
      headers: {
  ...
  if (!API_TOKEN) {
    return (
      <div className="min-h-screen flex items-center justify-center p-8 text-center">
        <div>
          <p className="text-xl font-semibold mb-2">TMDB API 토큰이 설정되지 않았습니다.</p>
          <p className="text-gray-600">프로젝트 루트에 .env 파일을 만들고 VITE_TMDB_TOKEN 값을 설정한 뒤, 개발 서버를 재시작해주세요.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6 mb-8">
        {movies?.map((movie) => (
          ...
          >
            ...
            {/* 호버 시 나타나는 정보 */}
            <div className="absolute inset-0 bg-gray-900 bg-opacity-0 group-hover:bg-opacity-70 transition-all duration-300 flex flex-col justify-center items-center p-4 opacity-0 group-hover:opacity-100">
